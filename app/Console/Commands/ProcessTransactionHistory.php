...
class ProcessTransactionHistory extends Command
{
    ...
    public function handle()
    {
        ...
        for ($day = 1; $day <= ($date[2]-1); ++$day) {
            $day_sale_sum = DB::select("SELECT SUM(sale_amount) AS 'sum' FROM transactions WHERE date BETWEEN '2016-03-" . $day . "' AND '2016-" . $date[2] . "-" . ($day + 1) . "'; ");

            $day_commission_sum = DB::select("SELECT SUM(commission) AS 'commission' FROM transactions WHERE date BETWEEN '2016-03-" . $day . "' AND '2016-" . $date[2] . "-" . ($day + 1) . "'; ");

            $transaction_count = DB::select("SELECT COUNT(*) AS 'count' FROM transactions WHERE date BETWEEN '2016-03-" . $day . "' AND '2016-" . $date[2] . "-" . ($day + 1) . "'; ");

            $last_transaction_id = DB::select("SELECT id FROM transactions WHERE date BETWEEN '2016-03-" . $day . "' AND '2016-" . $date[2] . "-" . ($day + 1) . "' ORDER BY id DESC LIMIT 1; ");
